...
class SDContributorsTest extends TestCase {
    ...
    public function testSaveMetaBox() {
        ...
        \WP_Mock::userFunction('get_post_meta', array(
            'args' => array( $post_id, 'sd_contributors', true ),
            'times' => 1,
            'return' => array( 3 ),
        ));
        ...
        \WP_Mock::userFunction('update_post_meta', array(
            'args' => array( $post_id, 'sd_contributors', $sd_authors ),
            'times' => 1,
        ));
        ...
        \WP_Mock::userFunction('update_user_meta', array(
            'times' => 2,
            'return_in_order' => array(
                array( 1, 'sd_contributor_post', array( $post_id ) ), 
                array( 2, 'sd_contributor_post', array( $post_id ) )
            ),
        ));
        
        // Created object of a call
        $contributors_object = new Sd_Contributors_Admin('sd-contributors', '1.0.0');

        // call save meta box function
        $contributors_object->save_meta_box($post_id);
    }
}
